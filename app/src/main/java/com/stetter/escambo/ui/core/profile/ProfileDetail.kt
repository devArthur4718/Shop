...
class ProfileDetail : BaseActivity() {
    ...
    private fun onProfileImageReceived(imgUrl: String) {
        ...
        if(imgUrl.length > 1){
            val gsReference = storage.getReferenceFromUrl("gs://escambo-1b51d.appspot.com/${imgUrl}")
            GlideApp.with(this)
                .load(gsReference)
    ...
    private fun onUserDataReceveid(userdata: RegisterUser?) {
        ...
        if(userdata!!.photoUrl.length > 1){
            val gsReference = storage.getReferenceFromUrl("gs://escambo-1b51d.appspot.com${userdata.photoUrl}")
            GlideApp.with(this)
                .load(gsReference)
                .placeholder(this?.CircularProgress())
                .into(binding.ivDetailProfileImage)
